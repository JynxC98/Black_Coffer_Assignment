...
for i in range(0,len(Word_list)-1):
    if Word_list["Negative"][i]>0:
        negative_words.append(Word_list["Word"][i]) #Negative and positive words are assigned values greater than 0.
...
def Web_Scraper(data,positive,negative,stop_words,personal_pronouns,class_name):
    ...
    for i in data.URL:
        ...
        """
        positive,negative,Polarity,subjectivity=Extracting_derived_variables(temp1,positive_words,negative_words,stop_words)
        ...
        """
        Avg_sent_length,Avg_words_per_sent,numb_sent=Analysis_of_readability(temp1,positive_words,negative_words,stop_words)
        Average_sentence_length.append(Avg_sent_length)
        Avg_word_length.append(Avg_words_per_sent)
        Sentence_Score.append(numb_sent)
        ...
        """
        word_count_bc,word_count_ac,syllable_count,personal_pronoun=Counts(temp1,positive_words,negative_words,stop_words,personal_pronouns)
        words_before_cleaning.append(word_count_bc)
        words_after_cleaning.append(word_count_ac)
        Syllable_count.append(syllable_count)
        Personal_pronoun_count.append(personal_pronoun)
    
    
    #Converting the lists into the appropriate dataframes.    
    data["Number of Sentences"]=Sentence_Score
    data["Number of positive words"]=Positive_Score
    data["Number of Negative words"]=Negative_Score
    data["Number of words before cleaning"]=words_before_cleaning
    data["Number of words after cleaning"]=words_after_cleaning
    data["Average_word_length"]=Avg_word_length
    data["Average sentence length"]=Average_sentence_length
    data["Subjectivity score"]=Subjectivity_score
    data["Syllable Count"]=Syllable_count
    data["Personal Pronoun"]=Personal_pronoun_count
    return data
    


def Extracting_derived_variables(appended_list,positive_words,negative_words,stop_words):
    
    
    sentence="".join(map(str,appended_list)).lower()
    #Calculating the number of positive words.
    number_of_positive=len([i for i in list(sentence.split(" ")) if i in positive_words and i not in stop_words])
    #Calculating the number of negative words.
    number_of_negative=len([i for i in list(sentence.split(" ")) if i in negative_words and i not in stop_words])
    #calculating polarity score
    if number_of_positive==0 and number_of_negative==0:
        polarity_score=0
    else:
        polarity_score=float((number_of_positive-number_of_negative)/(number_of_positive+number_of_negative)+0.000001)
    #calculating subjectivity score.
    number_of_words_after_cleaning=len([i for i in list(sentence.split(" ")) if i not in stop_words])
    subjectivity_score=float((number_of_positive+number_of_negative)/number_of_words_after_cleaning+0.000001)
...
def Counts(appended_list,positive_words,negative_words,stop_words,personal_pronouns):
    vowels=["a","e","i","o","u"]
    sentence="".join(map(str,appended_list)).lower()
    #calculating number of personal pronouns.
    number_of_personal_pronouns=len([i for i in list(sentence.split(" ")) if i in personal_pronouns])
    #Syllable_count
    Syllable_count=0
    ...
    number_of_words_after_cleaning=len([i for i in list(sentence.split(" ")) if i not in stop_words])
    return number_of_words_before_cleaning,number_of_words_after_cleaning,Syllable_count,number_of_personal_pronouns
    


updated_dataframe=Web_Scraper(data,positive_words,negative_words,stop_words,personal_pronouns,"td-post-content")
